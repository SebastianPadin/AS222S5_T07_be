...
public class TranslatorTextRest {
	...
	@GetMapping("/preview")
	public Mono<ResponseEntity<String>> previewTranslation(
			@RequestParam String text,
			@RequestParam String from,
			@RequestParam String to) {
		return translatorTextService.previewTranslation(text, from, to)
				.map(previewText -> ResponseEntity.ok(previewText))
				.onErrorResume(error -> {
					log.error("Error previewing translation: {}", error.getMessage());
					return Mono.just(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build());
				});
	}
	...
	public Mono<ResponseEntity<String>> deleteTranslationPermanently(@PathVariable Long id) {
		return translatorTextService.deleteTranslation(id)
				.map(success -> {
					if (success) {
						return ResponseEntity.ok("Translation physically deleted successfully");
					} else {
